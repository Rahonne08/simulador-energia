...
const Counter = ({ icon, label, value, onChange, min = 0 }: CounterProps) => (
  <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800">
    <div className="flex items-center gap-3">
      ...
      <span className="font-medium text-slate-700 dark:text-slate-300 text-sm sm:text-base">{label}</span>
    </div>
    <div className="flex items-center gap-3">
      <motion.button 
        ...
        className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 transition-all shadow-sm"
        aria-label={`Diminuir ${label}`}
      >
        <Minus className="w-4 h-4" />
      </motion.button>
      <span className="w-6 text-center font-bold text-slate-800 dark:text-slate-100">{value}</span>
      <motion.button 
        whileHover={{ scale: 1.1 }}
        whileTap={{ scale: 0.9 }}
...
export const QuickEstimate = () => {
  ...
  return (
    <div className="p-4 sm:p-6">
      <div className="mb-6">
        <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100">Estimativa Inteligente</h2>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Cálculo rápido baseado no perfil da residência</p>
      </div>
      ...
      <div className="bg-indigo-600 rounded-2xl p-6 sm:p-8 text-white relative overflow-hidden shadow-lg shadow-indigo-200 dark:shadow-none">
